...
describe('Vehicles', function () {
    ...
    it('should delete existing vehicle', async () => {
        const authTok = await token();
        const resCustomer = await request(app).post('/api/vehicles')
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                vin: "23212542d",
                regNo: "423r23er",
        ...
        const res = await request(app).del(`/api/vehicles/${resCustomer.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send();
        expect(res.statusCode).toEqual(200);
        expect(res.body).toHaveProperty('id');
    ...
    it('should update existing vehicle', async () => {
        ...
        const res = await request(app).put(`/api/vehicles/${resCustomer.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                status: "RUNNING"
            });
