...
describe('Customers', function () {
    ...
    it('should delete existing customer', async () => {
        const authTok = await token();
        const resCustomer = await request(app).post('/api/customers')
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                name: "Ahmed Abdelhay",
                address: "Cairo, Egypt"
            });
        const res = await request(app).del(`/api/customers/${resCustomer.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send();
        expect(res.statusCode).toEqual(200);
        expect(res.body).toHaveProperty('id');
    ...
    it('should update existing customer', async () => {
        ...
        const res = await request(app).put(`/api/customers/${resCustomer.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                address: "Giza, Egypt"
            });
