...
describe('Users', function () {
    ...
    it('should delete existing user', async () => {
        const authTok = await token();
        const resUser = await request(app).post('/api/users')
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                name: "Ahmed Abdelhay",
                email: "ahmed@mido.com",
        ...
        const res = await request(app).del(`/api/users/${resUser.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send();
        expect(res.statusCode).toEqual(200);
        expect(res.body).toHaveProperty('id');
    ...
    it('should update existing user', async () => {
        const authTok = await token();
        const resCustomer = await request(app).post('/api/users')
            .set({Authorization: `Bearer ${authTok}`})
            .send({
                name: "Ahmed Abdelhay",
                email: "ahmed@mido.com",
        ...
        const res = await request(app).put(`/api/users/${resCustomer.body.id}`)
            .set({Authorization: `Bearer ${authTok}`})
            .send({
